...
struct Task {
  ...
  Task(int _id, int _priority, int period_ms, int exec_time_ms,
       std::function<void(int)> _work)
      : id(_id), priority(_priority),
        period(std::chrono::milliseconds(period_ms)),
        execution_time(std::chrono::milliseconds(exec_time_ms)),
        next_deadline(std::chrono::system_clock::now()),
        work([_id, this, _work]() { _work(id); }) {
    std::printf(
        "Created task %d: Priority=%d, Period=%ldms, ExecTime=%ldms at %s\n",
...
struct TaskComparator {
  bool operator()(const Task *t1, const Task *t2) const {
    if (t1->next_deadline == t2->next_deadline)
      return t1->priority <
             t2->priority; // Same deadline, higher priority first
    ...
    while (true) {
      ...
      for (auto &task : tasks) {
        if (task.next_deadline <= now) {
          std::printf("Task %d ready at %s\n", task.id,
                      time_utils::formatTime(now).c_str());
      ...
      if (!ready_queue.empty()) {
        ...
        std::printf("Executing task %d at %s\n", task->id,
                    time_utils::formatTime(start_time).c_str());
        task->work();
        ...
        if (end_time > task->next_deadline) {
          std::printf("Task %d missed deadline at %s\n", task->id,
                      time_utils::formatTime(end_time).c_str());
        } else {
        ...
        if (!tasks.empty()) {
          ...
          cv.wait_until(lock, next_deadline,
                        [this]() { return !running || !ready_queue.empty(); });
        } else {
...
public:
  ...
  void addTask(int id, int priority, int period_ms, int exec_time_ms,
               std::function<void(int)> work) {
    std::lock_guard<std::mutex> lock(mtx);
    tasks.emplace_back(id, priority, period_ms, exec_time_ms, work);
    cv.notify_one();
    std::printf(
        "Added task %d to scheduler at %s\n", id,
        time_utils::formatTime(std::chrono::system_clock::now()).c_str());
  }
...
int main() {
  ...
  scheduler.addTask(1, 2, 1000, 200,
                    sensorReading); // Sensor reading every 1s, takes 200ms
  scheduler.addTask(2, 1, 2000, 300,
                    controlLoop); // Control loop every 2s, takes 300ms
  scheduler.addTask(
      3, 3, 500, 100,
      sensorReading); // High-priority sensor every 500ms, takes 100ms
