...
private:
  ...
  SensorData read_sensor(const char *sensor_name, int sensor_id) {
    ...
    SensorData data;
    data.value = dis(gen);
    data.timestamp = steady_clock::now();
    std::printf("[DEBUG] Sensor %s (ID: %d) read value: %.2f at time %ld\n",
  ...
  void fuse_data() {
    ...
    // Update shared state
    {
      ...
      state.last_updated = steady_clock::now();
      std::printf("[DEBUG] Fused state updated: value = %.2f at time %ld\n",
                  state.fused_value,
                  state.last_updated.time_since_epoch().count());
    }
    sensor_readings.clear(); // Clear readings after fusion
  }

public:
  SensorFusion() : state{0.0, steady_clock::now()} {}
  ...
  void sensor_thread(const char *sensor_name, int sensor_id) {
    while (running) {
      ...
      { // Store reading in buffer
        ...
        std::printf(
            "[DEBUG] Sensor %s (ID: %d) stored reading, buffer size: %zu\n",
  ...
  void fusion_thread() {
    while (running) {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(200)); // Fuse every 200ms
    }
...
int main() {
  ...
  std::printf(
      "compile: g++ -std=c++23 <file_name.cpp> -o sensor_fusion -pthread\n");

  SensorFusion fusion;
  std::vector<std::jthread> threads;

  // Step 5: Start sensor threads (IMU, Camera, Lidar).
  threads.emplace_back([&fusion](int id) { fusion.sensor_thread("IMU", id); },
                       1);
  threads.emplace_back(
      [&fusion](int id) { fusion.sensor_thread("Camera", id); }, 2);
  threads.emplace_back([&fusion](int id) { fusion.sensor_thread("Lidar", id); },
                       3);
  ...
  std::printf("[DEBUG] Final fused state: value = %.2f at time %ld\n",
              final_state.fused_value,
              final_state.last_updated.time_since_epoch().count());
