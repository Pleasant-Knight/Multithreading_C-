...
void eat() {
  ...
  std::this_thread::sleep_for(
      std::chrono::milliseconds(90)); // Simulate eating time
}
...
int main() {
  std::printf("Compile: g++ -std=c++11 dining_philosophers.cpp -o "
              "dining_philosophers.app -pthread\n");
  ...
  for (int i = 0; i < 5; ++i) {
    threads.emplace_back([i, &dp]() {
      for (int j = 0; j < 3; ++j) {
        dp.wantsToEat(i, pickLeftFork, pickRightFork, eat, putLeftFork,
                      putRightFork);
