/*
 ...
 The key idea is to:
  ...
  ensuring meetings remain within [0, eventTime] and maintain their relative order.
 Explanation:
 Data Structure: 
  We create a vector of pairs to store meeting start and end times, ensuring we can sort 
  them if needed (though the problem states they are non-overlapping and ordered).
 Initial Gaps: Calculate gaps between consecutive meetings, including the gap from time 0 
  to the first meeting and from the last meeting to eventTime.
 Rescheduling Strategy:
    For each meeting, we try moving it to its earliest possible start time 
	(right after the previous meeting or 0 if it's the first) and its latest possible start 
	time (right before the next meeting or eventTime if it's the last).
    For each move, we recalculate the affected gaps (before and after the moved meeting) 
	and find the maximum gap possible.
    We only consider up to k moves, and for simplicity, this solution evaluates single moves 
	(since k=1 in the examples).
*/
...
int maxFreeTime_recursive(int eventTime, int k, std::vector<int>& startTime, std::vector<int>& endTime) {
    ...
    std::function<void(int, int, std::vector<int>&)> tryCombinations = 
        [&](int pos, int movesLeft, std::vector<int>& starts) {
            if (movesLeft == 0 || pos >= n) {
                ...
                maxFree = std::max(maxFree, *std::max_element(tempGaps.begin(), tempGaps.end()));
                return;
            }
            ...
            int earliestStart = (pos == 0) ? 0 : meetings[pos-1].second - (meetings[pos-1].first - starts[pos-1]);
...
int maxFreeTime(int eventTime, int k, std::vector<int>& startTime, std::vector<int>& endTime) {
    ...
    for (int start = 0; start < n; ++start) {
        int moves = k;
        std::vector<int> tempStarts = startTime; // Temporary copy for this iteration
        int prevEnd = 0;
        bool valid = true;
        ...
        for (int i = start; i < n && moves > 0; ++i) {
            ...
            int earliestStart = (i == 0) ? 0 : endTime[i-1] - (startTime[i-1] - tempStarts[i-1]);
            if (tempStarts[i] > earliestStart) {
                tempStarts[i] = earliestStart;
                --moves;
            }
            if (i > 0 && tempStarts[i] < endTime[i-1] - (startTime[i-1] - tempStarts[i-1])) {
    ...
    for (int end = n - 1; end >= 0; --end) {
        ...
        for (int i = end; i >= 0 && moves > 0; --i) {
            int duration = endTime[i] - startTime[i];
            int latestStart = (i == n-1) ? eventTime - duration : tempStarts[i+1] - duration;
