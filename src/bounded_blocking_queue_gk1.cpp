/*
 Design Bounded Blocking Queue problem:
   ...
   The queue supports three methods:
         ...
         - dequeue (removes and returns an element, blocking if the queue is
 empty), and
         - size (returns the current number of elements).
   The implementation must handle concurrent access correctly, ensuring thread
 safety and proper blocking behavior.
*/
...
int main() {
  std::printf("Compile: g++ -std=c++11 bounded_blocking_queue_gk1.cpp -o "
              "bounded_blocking_queue -pthread\n");
