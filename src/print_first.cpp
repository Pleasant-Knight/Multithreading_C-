...
int main() {
    ...
    std::vector<std::pair<std::string, std::function<std::thread()>>> thread_creators = {
        {"first", [&]() { return std::thread([&]() { foo.first(printFirst); }); }},
        {"second", [&]() { return std::thread([&]() { foo.second(printSecond); }); }},
        {"third", [&]() { return std::thread([&]() { foo.third(printThird); }); }}
    };
