...
void sensor_thread(SharedData &shared) {
  ...
  while (running.load(std::memory_order_relaxed)) {
    double local_plant_state =
        shared.plant_state.load(std::memory_order_acquire);
    std::this_thread::sleep_for(
        std::chrono::milliseconds(500)); // Simulate sensor delay
...
void controller_thread(SharedData &shared, PID &pid) {
  ...
  while (running.load(std::memory_order_relaxed)) {
    double local_sensor_value =
        shared.sensor_value.load(std::memory_order_acquire);
    ...
    std::printf("Controller thread %s: sensor_value = %f, output = %f, "
                "plant_state = %f\n",
...
int main() {
  ...
  if (!shared.plant_state.is_lock_free() ||
      !shared.sensor_value.is_lock_free()) {
    std::printf(
        "Error: std::atomic<double> is not lock-free on this platform\n");
    return 1;
  }
